...
function getHeaderMap(csvData, title) {
  return new Promise((resolve) => {
    Papa.parse(csvData, {
      complete: function(results) {
        if (results.data.length > 0) {
          let headerRow = results.data.find(row => row.some(cell => cell.includes('お客様名') || cell.includes('氏名') || cell.includes('ユーザーID') || cell.includes('受付日') || cell.includes('日時')));
